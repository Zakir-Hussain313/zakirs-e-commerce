...
export async function PUT(request: NextRequest) {
  try {
    ...
    const data = await ProductVarientModel.find({ _id: { $in: ids } }).lean();
    if (!data.length) {
    ...
    // ✅ Perform update
    if (deleteType === "SD") {
      await ProductVarientModel.updateMany(
        { _id: { $in: ids } },
...
export async function DELETE(request: NextRequest) {
  ...
  try {
    ...
    const data = await ProductVarientModel.find({ _id: { $in: ids } })
      .lean();
    ...
    await ProductVarientModel.deleteMany({ _id: { $in: ids } });
